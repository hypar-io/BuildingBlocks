...
namespace Elements
{
	...
	public partial class MechanicalCorridor : GeometricElement
    {
        ...
        public MechanicalCorridor(Polygon @perimeter, Vector3 @direction, double @rotation, Vector3 @start, Vector3 @end, double @length, double @volume, string @number, Transform @transform, Material @material, Representation @representation, System.Guid @id, string @name)
            : base(transform, material, representation, id, name)
        {
            ...
            if(validator != null)
            {
                validator.Validate(new object[]{ @perimeter, @direction, @rotation, @start, @end, @length, @volume, @number, @transform, @material, @representation, @id, @name});
            }
